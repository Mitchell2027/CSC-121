#Program of a company database using classes (functions.py)
#Mariah Mitchell
#CSC 121 0901
#12/10/2025
...
# ----------------- Employee Class -----------------
class Employee:
    def __init__(self, name, emp_id, email, phone, hire_date, position, salary):
        self._name = name
        self._emp_id = emp_id
        self._email = email
        self._phone = phone
...
# ----------------- Department Class -----------------
class Department:
    def __init__(self, dep_name, budget, manager, location, employees=None):
        self._dep_name = dep_name
        self._budget = budget
        self._manager = manager  # Must be Employee instance
    ...
    def set_manager(self, manager):
        if isinstance(manager, Employee):
            ...
            raise ValueError("Manager must be an Employee instance.")
    ...
    # ---------- Representation ----------
    def __repr__(self):
        return (f"Department: {self._dep_name}\n"
                f"Budget: ${self._budget:,.2f}\n"
                f"Location: {self._location}\n"
                f"Manager: {self._manager.get_name()} (${self._manager.get_salary():,.2f})\n"
...
# ----------------- Instance Creation -----------------
def create_instances(company_db):
    '''

    Parameters
    ----------
    company_db : dict
       Nested dictionary containing department and employee data.

    Returns
    -------
    emp_instances : dict
        Dictionary of Employee instances keyed by employee ID.
    dept_instances : dict
        Dictionary of Department instances keyed by department name.
    '''
    emp_instances = {}
    dept_instances = {}
    ...
    for dept_name, dept_data in company_db.items():
        for emp_id, emp_data in dept_data["employees"].items():
            ...
            emp_instances[emp_id.lower()] = e

    # ------------------------------
    # 2. CREATE DEPARTMENT INSTANCES
    # ------------------------------
    for dept_name, dept_data in company_db.items():
        info = dept_data["department_info"]

        manager_id = info["manager"].lower()
        manager_instance = emp_instances.get(manager_id)
        ...
        # Add employees
        for emp_id in dept_data["employees"]:
            employee_instance = emp_instances.get(emp_id.lower())
            if employee_instance:
                d.add_employee(employee_instance)

        dept_instances[info["name"].lower()] = d

    return emp_instances, dept_instances


def write_employee(emp_list, dept_name):
    '''
    Write employee data for a specific department into a CSV file.

    Parameters
    ----------
    emp_list : list
        A list of Employee instances belonging to the department.
    dept_name : str
       The name of the department. Used to generate the CSV filename.
    ...
    '''
    filename = dept_name.lower() + "_employees.csv"

    with open(filename, "w", newline="") as file:
        writer = csv.writer(file)

        writer.writerow(["Employee ID", "Name", "Email", "Phone", "Hire Date", "Position", "Salary"])
...
def write_department(dept):
    '''
    Create a text file containing information about a department.

    Parameters
    ----------
    dept : Department
        A department object with accessor methods for name, budget,
    ...
    filename = dept.get_dep_name().lower() + ".txt"
    with open(filename, "w") as file:
        file.write(f"Department Name: {dept.get_dep_name()}\n")
        file.write(f"Budget: ${dept.get_budget():,.2f}\n")
        file.write(f"Location: {dept.get_location()}\n")
        manager = dept.get_manager()
        file.write(f"Manager ID: {manager.get_emp_id()}\n")
        file.write(f"Manager Name: {manager.get_name()}\n")
    print(f"{filename} has been created.")
